...
def _mapping_folder(folder, mapping, original_directory):
    ...
    groups_secrets = False
    if (working_dir.split("/")[-1] == 'groups') or (working_dir.split("/")[-1] == 'secret_scopes'):
        groups_secrets = True

    files_in_folder = os.listdir(working_dir)
    for file in files_in_folder:
        if (file.lower() == 'artifacts') or (file.lower() == "metastore") or (file.lower() == "errors") or (file.lower() == "app_logs"):  # do not map any notebooks or metastore
            continue
        if ("." not in file) and not groups_secrets: #assuming extension is not there -> folder
